    ...
    override fun isAuthenticated(): Boolean {
        try {
            ...
            httpClient.newCall(request).execute().use { response ->
                val body =
                    response.body?.string() ?: throw FeverApiException(
    ...
    override fun items(): FeedItemList {
        try {
            ...
            httpClient.newCall(request).execute().use { response ->
                ...
                if (moshi.adapter(FeverAuthResponse::class.java).fromJson(body)?.auth != 1) {
                    throw FeverApiException("Invalid username or password")
                }

                val items = moshi.adapter(FeedItemList::class.java).fromJson(body)
                return items!!
            }
        } catch (e: IllegalArgumentException) {
            throw FeverApiException(
